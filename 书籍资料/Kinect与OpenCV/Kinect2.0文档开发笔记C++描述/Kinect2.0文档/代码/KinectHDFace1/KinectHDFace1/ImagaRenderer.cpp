...
void ImageRenderer::draw_facedata(){
    ...
    // 临时设为红色笔刷
    m_pWhiteBrush->SetColor(D2D1::ColorF(D2D1::ColorF::Red));
    // 刻画点
    D2D1_ELLIPSE ellipse = { {0}, 3.f, 3.f };
    for (UINT i = 0U; i < this->data.face_points_count; ++i){
        ellipse.point.x = this->data.face_points[i].X;
        ellipse.point.y = this->data.face_points[i].Y;
        m_pRenderTarget->FillEllipse(ellipse, m_pWhiteBrush);
    }
    // 还原
    m_pWhiteBrush->SetColor(D2D1::ColorF(D2D1::ColorF::White));
    /*// 外框
    D2D1_RECT_F rect = { 0 };
    // 刻画...
    WCHAR buffer[64];
    const auto row = 5;
    const auto width = 128.f;
    const auto height = 40.f;
    for (int i = 0; i < lengthof(this->data.sd); ++i){
        rect.left = static_cast<float>(i % 5) * width + 0.5f * static_cast<float>(WNDWIDTH);
        rect.top = static_cast<float>(i / 5) * height + 0.5f * static_cast<float>(WNDHEIGHT);
        rect.right = rect.left + width;
        rect.bottom = rect.top + height;
        auto length = swprintf_s(buffer, L"%03.03f", this->data.sd[i]);
