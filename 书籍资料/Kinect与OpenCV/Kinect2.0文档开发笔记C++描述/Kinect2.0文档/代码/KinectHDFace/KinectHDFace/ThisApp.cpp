...
void ThisApp::check_hd_face_frame(){
    ...
    // 获取面部外框
    if (SUCCEEDED(hr)){
        hr = m_pFaceAlignment->get_FaceBoundingBox(&m_ImagaRenderer.data.face_box);
    }
    // 获取面部旋转四元数
    if (SUCCEEDED(hr)){
        hr = m_pFaceAlignment->get_FaceOrientation(&m_ImagaRenderer.data.face_rotation);
    }
    // 获取头部旋转中心
    if (SUCCEEDED(hr)){
        CameraSpacePoint point;
        hr = m_pFaceAlignment->get_HeadPivotPoint(&point);
        // 顺便转换为彩色帧坐标
        if (SUCCEEDED(hr)){
            hr = m_pMapper->MapCameraPointToColorSpace(point, &m_ImagaRenderer.data.head_pivot_2);
        }
    }
    // 获取AU数据
    if (SUCCEEDED(hr)){
        hr = m_pFaceAlignment->GetAnimationUnits(FaceShapeAnimations_Count, m_ImagaRenderer.data.au);
    }
    // 检查跟踪
    if (SUCCEEDED(hr)){
